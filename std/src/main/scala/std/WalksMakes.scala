...
object Makes extends StdMakes {
  ...
  def direct[A](xs: A*): Direct[A]                              = fromInts(xs.apply, 0, xs.length)
  def fromInts[A](f: Int => A, start: Int, end: Int): Direct[A] = new FromInts(f, start, end)
  def continually[A](expr: => A): Indexed[A]                    = new Continual(expr)
...
trait StdWalks {
  ...
  private def fromScala[A](xs: GTOnce[A]): Each[A] = xs match {
    ...
    case xs: sCollection[A]  => Cont[A](xs foreach _).suspend
...
trait StdConstructors {
  ...
  def suspend[A](mfs: Suspended[A]*): Each[A]         = Cont[A](f => mfs foreach (_ apply f)).suspend
