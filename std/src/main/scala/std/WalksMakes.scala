...
trait StdWalks {
  ...
  def jvmArray[A]: Walks[A, Array[A]]                              = Walks(xs => fromInts(xs.apply, 0, xs.length))
  def jvmString: Walks[Char, String]                               = Walks(s => fromInts(s charAt _, 0, s.length))
  ...
  private def fromInts[A, R](f: Int => A, start: Int, end: Int): Direct[A] = new FromInts(f, start, end)
  private class FromInts[A](f: Int => A, start: Int, end: Int) extends StdDirect[A](Size(end - start)) {
    def apply(idx: Vdex): A = f(start + idx.indexValue.toInt)
    def reverse: Direct[A]  = fromInts(n => f(end - 1 - n), 0, size.getInt)
  }
}
trait StdMakes {
  ...
  def pspMap[K: Eq, V]: Makes[K->V, Pmap[K, V]] = pspVec[K->V] map (kvs => kvs.m.toMap[sciMap](scalaMap) |> (f => Pmap(kvs map fst toPset, Fun(f))))
  def pspSet[A: Eq]: Makes[A, Pset[A]]          = Makes(Pset apply _)
  def pspVec[A]: Makes[A, Vec[A]]               = scalaVector[A] map (xs => new Vec(xs))
