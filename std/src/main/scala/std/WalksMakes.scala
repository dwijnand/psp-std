...
trait StdMakes {
  ...
  def javaGenericMap[K, V, M[X, Y] <: jMap[X, Y]](implicit z: Empty[M[K, V]]): Makes[K -> V, M[K, V]] =
    create[K -> V](z.empty)((r, kv) => kv app r.put)(identity)
  ...
  def pspMap[K: Hash: Eq, V]: Makes[K -> V, Pmap[K, V]] =
    pspDirect[K -> V] map (kvs => kvs.m.toMap[sciMap](scalaMap) |> (f => Pmap(kvs map fst toPset, Fun(f))))
